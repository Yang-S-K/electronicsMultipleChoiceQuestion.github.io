...
async function loadWordBank() {
  try {
    const response = await fetch("power_amplifier_questions.json");
    ...
    alert("無法載入預設題庫，請確認 power_amplifier_questions.json 是否存在！");
  }
}
...
function handleChoice(item, userAnswerIndex) {
  ...
  if (item.explanation) {
    ...
    explanation.innerText = `解析：${item.explanation}`;
...
function showRecordDetail(index, page = 1) {
  ...
  currentAnswers.forEach((ans, i) => {
    ...
    item.innerHTML = `<p>第 ${start + i + 1} 題：${ans.question}<br>你的答案：${ans.userAnswer}<br>正確答案：${ans.correctAnswer}<br>${ans.isCorrect ? "✅" : "❌"}<br>解析：${ans.explanation || "（無）"}</p>`;
...
function renderFilteredWordBank(keyword, page = 1) {
  ...
  currentQuestions.forEach((item, index) => {
    const div = document.createElement("div");
    div.innerHTML = `<p><strong>#${start + index + 1}</strong><br>${item.question}<br>選項：${item.options.join("，")}<br>正解：${item.options[item.answer]}<br>解析：${item.explanation || "（無）"}</p>`;
